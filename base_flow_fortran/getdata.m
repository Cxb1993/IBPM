...
function [xb,yb,codeb,xn,yn,un,vn,u0pn,v0pn,u0rn,v0rn,wn,sn,pn,filename] = getdata(dir,it,lev,pressure)
...
% ---- Open file ----------------------------------------------------------
filename = [dir '/ib_lin' num2str(it,'%7.7i') '.var'];
...
temp = fread(fid, 1, 'float32');      % Fortran marker
	xb0 = fread(fid,1,'real*8' );   % rotating angle of the frame
	u_ib = fread(fid,1,'real*8' );     % x-coordinate of the center of rotation
	ud_ib = fread(fid,1,'real*8' );     % y-coordinate of the center of rotation
    udd_ib = fread(fid,1,'real*8' ); 
temp = fread(fid, 1, 'float32');      % Fortran marker
